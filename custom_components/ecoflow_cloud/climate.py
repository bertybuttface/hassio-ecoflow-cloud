...
class Wave2ClimateEntity(ClimateEntity, EcoFlowAbstractEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        # Update HVAC mode based on power mode and main mode
        power_mode = data.get("pd.powerMode")
        main_mode = data.get("pd.mainMode")
        
        if power_mode == 3 or power_mode == 2:  # Shutdown or Standby
            self._hvac_mode = HVACMode.OFF
        elif power_mode == 1 and main_mode is not None:  # Startup
            if main_mode == 0:  # Cool
        ...
        self._attr_available = any(key in data for key in ["pd.setTemp", "pd.powerMode", "pd.envTemp"])
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        if hvac_mode == HVACMode.OFF:
            # Set to shutdown
            self._send_command("pd.powerMode", 3, "powerMode", "powerMode")
        else:
            # First ensure device is on (startup mode)
